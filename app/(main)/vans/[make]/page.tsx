...
const CATEGORY_SLUGS = new Set([
  ...
  "pickup", "minibus", "electric", "new", "used",
]);
      ...
      <section className="border-b border-border bg-surface-1">
        <Container className="py-10">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <Link
              href={`/vans?condition=new&q=${encodeURIComponent(makeName)}`}
            ...
            <Link
              href={`/vans?condition=used&q=${encodeURIComponent(makeName)}`}
              className="rounded-[var(--radius-pill)] border border-border bg-white px-5 py-2 text-[var(--text-sm)] font-semibold text-ink-700 hover:border-ink-400"
            >
              Used {makeName}
            </Link>
      ...
      <Container className="py-8">
        {result.listings.length === 0 ? (
          <div className="rounded-[var(--radius-lg)] border border-dashed border-border-strong bg-card p-10 text-center">
            ...
            <p className="mx-auto mt-2 max-w-sm text-[var(--text-sm)] text-ink-500">
              Check back soon — stock updates regularly.
            </p>
        ...
        {result.total > 24 && (
          <div className="mt-8 text-center">
            <Link
              href={`/vans?q=${encodeURIComponent(makeName)}`}

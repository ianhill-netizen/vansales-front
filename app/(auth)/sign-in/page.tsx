...
// Metadata can't be used in client components — set in a separate server wrapper if needed
...
export default function SignInPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // Mock: admin email → admin persona, else buyer
    ...
    <div className="w-full max-w-sm">
      <div className="overflow-hidden rounded-[var(--radius-xl)] border border-border bg-white shadow-[var(--shadow-md)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <div>
            <label htmlFor="email" className="block text-[var(--text-xs)] font-semibold uppercase tracking-widest text-ink-500">
              Email address
            </label>
            <input id="email" name="email" type="email" required autoComplete="email" placeholder="you@example.com"
              className="mt-1.5 h-11 w-full rounded-[var(--radius-md)] border border-border bg-surface-0 px-3 text-[var(--text-base)] text-ink-800 outline-none focus-visible:border-brand-500 focus-visible:ring-2 focus-visible:ring-brand-500/15" />
          </div>
          <div>
            <div className="flex justify-between">
              <label htmlFor="password" className="text-[var(--text-xs)] font-semibold uppercase tracking-widest text-ink-500">
                Password
              </label>
      ...
      <p className="mt-5 text-center text-[var(--text-sm)] text-ink-500">
        New to Vansales?{" "}
        <Link href="/sign-up" className="font-semibold text-brand-700 hover:underline">Create an account →</Link>
      </p>
      <p className="mt-3 text-center text-[var(--text-xs)] text-ink-400">
        Are you a dealer?{" "}
        <Link href="/dealer-portal/login" className="text-brand-600 hover:underline">Log in to Dealer Portal</Link>
      </p>
    </div>
  );
}

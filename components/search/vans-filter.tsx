...
/* =============================================================================
   VANS FILTER — pill row + single modal (AutoTrader-style)
   Used by /vans page.  Pill row is always visible; clicking any pill or the
   "Filter & sort" button opens the modal.  On "Search" / close: URL is updated
   and the server component re-renders with the new filters.
   ============================================================================= */
...
type ActivePill = { key: keyof FilterDraft | "make_model" | "price" | "year"; label: string };

function getActivePills(d: FilterDraft): ActivePill[] {
  const out: ActivePill[] = [];

  if (d.make) {
    const makeData = getMakeBySlug(d.make);
    const makeName = makeData?.name ?? ucFirst(d.make);
    const modelData = d.model ? makeData?.models.find((m) => m.slug === d.model) : null;
    out.push({ key: "make_model", label: modelData ? `${makeName} · ${modelData.name}` : makeName });
  } else if (d.model) {
    out.push({ key: "model", label: ucFirst(d.model) });
  }

  if (d.bodyStyle) out.push({ key: "bodyStyle", label: unslug(d.bodyStyle) });
  if (d.fuel)      out.push({ key: "fuel",      label: ucFirst(d.fuel) });
  if (d.gearbox)   out.push({ key: "gearbox",   label: ucFirst(d.gearbox) });
  if (d.wheelbase) out.push({ key: "wheelbase", label: WHEELBASE_SHORT[d.wheelbase as keyof typeof WHEELBASE_SHORT] ?? d.wheelbase.toUpperCase() });
  if (d.colour)    out.push({ key: "colour",    label: ucFirst(d.colour) });

  if (d.monthlyMax) {
    out.push({ key: "monthlyMax", label: `≤ £${d.monthlyMax}/mo` });
  } else if (d.minPrice || d.maxPrice) {
    const lo = d.minPrice ? `£${Number(d.minPrice).toLocaleString("en-GB")}` : null;
    const hi = d.maxPrice ? `£${Number(d.maxPrice).toLocaleString("en-GB")}` : null;
    out.push({ key: "price", label: lo && hi ? `${lo}–${hi}` : lo ? `${lo}+` : `up to ${hi}` });
  }
  ...
  if (d.maxMileage) {
    out.push({ key: "maxMileage", label: `< ${Number(d.maxMileage).toLocaleString("en-GB")} mi` });
  }

  if (d.sort && d.sort !== "newest") {
    const SORT_LABEL: Record<string, string> = {
      price_asc: "Price ↑", price_desc: "Price ↓", mileage_asc: "Mileage ↑",
    };
    out.push({ key: "sort", label: `Sort: ${SORT_LABEL[d.sort] ?? d.sort}` });
  }
...
const MONTHLY_PRESETS = ["200", "300", "400", "500"];
...
export function VansFilter({
  ...
  const pills = getActivePills(draft);
  ...
  function applyAndClose() {
    startTransition(() => {
      router.push(draftToUrl(draft), { scroll: false });
    });
    setModalOpen(false);
  }

  function clearPill(key: ActivePill["key"]) {
    let updates: Partial<FilterDraft> = {};
    if (key === "make_model")  { updates = { make: "", model: "" }; }
    else if (key === "price")  { updates = { minPrice: "", maxPrice: "", monthlyMax: "" }; }
    else if (key === "year")   { updates = { minYear: "", maxYear: "" }; }
    else if (key === "monthlyMax") { updates = { monthlyMax: "", maxPrice: "" }; }
    else if (key === "sort")   { updates = { sort: "newest" }; }
    else                       { updates = { [key as keyof FilterDraft]: "" }; }
    const next = { ...draft, ...updates };
    setDraft(next);
    startTransition(() => {
      router.push(draftToUrl(next), { scroll: false });
    });
  }
    ...
    <>
      <div className="mb-5 flex flex-wrap items-center gap-2">
        <button
          ...
          className={`flex items-center gap-2 rounded-[var(--radius-pill)] border px-4 py-2 text-[var(--text-sm)] font-semibold shadow-[var(--shadow-xs)] transition-colors ${
            pills.length > 0
        ...
        {pills.map((pill) => (
          <button
            ...
            className="flex items-center gap-1.5 rounded-[var(--radius-pill)] border border-brand-200 bg-brand-50 px-3 py-1.5 text-[var(--text-xs)] font-semibold text-brand-700 transition-colors hover:bg-brand-100"
          >
            {pill.label}
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" aria-label="Filter and sort">
          ...
          <div className="relative mx-auto flex max-h-[92dvh] w-full max-w-lg flex-col overflow-hidden bg-white shadow-[var(--shadow-xl)] md:my-16 md:rounded-[var(--radius-2xl)] rounded-t-[var(--radius-2xl)] mt-auto">
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <Section title="Monthly payment">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        set({
                          monthlyMax: v,
                          minPrice: "",
                          maxPrice: v ? String(priceFromMonthly(Number(v))) : "",
                        });
                      }}
                      placeholder="Max per month"
                      min="0"
                      step="50"
                      className="w-full rounded-[var(--radius-md)] border border-border py-2.5 pl-7 pr-12 text-[var(--text-sm)] font-semibold text-ink-900 placeholder:font-normal placeholder:text-ink-400 focus:border-brand-400 focus:outline-none"
                    />
                    <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-xs)] text-ink-400">/mo</span>
                  </div>
                  {draft.monthlyMax && Number(draft.monthlyMax) > 0 && (
                    <p className="shrink-0 text-[var(--text-xs)] text-ink-500">
                      ≈ up to{" "}
                      <span className="font-semibold text-ink-700">
                        £{priceFromMonthly(Number(draft.monthlyMax)).toLocaleString("en-GB")}
                      </span>
                    </p>
                  )}
                </div>
                    ...
                    <SmallChip
                      ...
                      onClick={() =>
                        set(
                          draft.monthlyMax === v
                            ? { monthlyMax: "", maxPrice: "" }
                            : { monthlyMax: v, minPrice: "", maxPrice: String(priceFromMonthly(Number(v))) },
                        )
                      }
                    >
                      ≤ £{v}/mo
                ...
                <p className="mt-3 text-[var(--text-2xs)] text-ink-400">
                  HP estimate: {FINANCE_ASSUMPTIONS.depositPct * 100}% deposit ·{" "}
                  {FINANCE_ASSUMPTIONS.termMonths} months · {FINANCE_ASSUMPTIONS.apr}% APR. Indicative only.
                </p>
              </Section>
